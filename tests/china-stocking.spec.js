...
test('NRP Stocking Logic: Certification Level vs Suggested Segment', async ({ page }) => {
  ...
  // 4.5️⃣ APPLY RULE FILTER: Product BU = CTT
  console.log('🔍 Applying Rule Filter: Product BU = CTT...');
  ...
  // 1. Click the first placeholder (Column Selector)
  await dialog.locator('mat-select').first().click();
  await page.waitForTimeout(500); // Wait for dropdown

  // 2. Search "Product BU"
  console.log('   Searching for column "Product BU"...');
  await page.keyboard.type('Product BU'); 
  await page.waitForTimeout(1000); 

  // 3. Select the second option (as per instruction) or exact match
  // We try to find the option. If "Product BU" search filtered the list, we pick from results.
  const options = page.locator('mat-option');
  if (await options.count() >= 1) {
      // User said "select the second option", assuming index 1.
      // If only one option appears (Product BU), selecting index 0 is safer, but adhering to instruction:
      ...
      // Fallback: Try clicking text directly if search didn't filter
      await page.getByText('Product BU').first().click();
  }

  // 4. Write "CTT" value
  console.log('   Entering value "CTT"...');
  const valueInput = dialog.locator('input').last(); // Usually the value input is last
  await valueInput.fill('CTT');
  ...
  // 5️⃣ FIND A CODE NOT ENDING WITH 'H' (POSTFIX)
  ...
  for (let i = 0; i < await headers.count(); i++) {
      ...
      if (title === 'Annual Volume') annualVolumeColIndex = i;
  }

  if (codeColIndex === -1 || certColIndex === -1 || segmentColIndex === -1 || annualVolumeColIndex === -1) {
      throw new Error('❌ Could not find one or more required columns.');
  }
  ...
  if (targetRowIndex === -1) {
      throw new Error('❌ No valid code found in the first page of results.');
  }
  ...
  // Logic: 
  // If (Cert is null/empty) AND (Annual Volume is null/empty/0) -> Expect 'D'
  // Else -> Expect 'A'
  
  const isCertNull = !certValue || certValue === '';
  const isVolumeNull = !volumeValue || volumeValue === '' || parseFloat(volumeValue) === 0;

  const expectedSegment = (isCertNull && isVolumeNull) ? 'D' : 'A';

  console.log(`   Logic: Cert(${isCertNull}) & Volume(${isVolumeNull}) -> Expecting Segment '${expectedSegment}'`);
