...
// function resolveFuturePVC(pvc) {
//   if (pvc.publishCode) {
//     return { code: pvc.publishCode, date: pvc.publishDate };
//   }
//   if (pvc.futureCode) {
//     return { code: pvc.futureCode, date: pvc.futureDate };
//   }
//   if (pvc.effectiveCode) {
//     return { code: pvc.effectiveCode, date: pvc.effectiveDate };
//   }
//   return null;
// }
...
console.log('✅ Allow Flag = Yes → continue PVC logic');
  ...
  const pvcAction = {
    ...
    publishDate: await page.locator('td[data-col="publishPVCEffectiveDate"]').textContent(),
    futureCode: await page.locator('td[data-col="futurePVC"]').textContent(),
    futureDate: await page.locator('td[data-col="futurePVCEffectiveDate"]').textContent(),
    effectiveCode: await page.locator('td[data-col="effectivePVC"]').textContent(),
    effectiveDate: await page.locator('td[data-col="effectivePVCEffectiveDate"]').textContent()
  };
  ...
  const futurePVC = (await chinaPage.locator('#futurePVCCode').textContent())?.trim();
  const futureDate = (await chinaPage.locator('#futurePVCEffectiveDate').textContent())?.trim();
  const currentPVC = (await chinaPage.locator('#currentPVCCode').textContent())?.trim();
  const publishPVC = (await chinaPage.locator('#publishPVCCode').textContent())?.trim();

  if (selectedPVC.code === currentPVC || selectedPVC.code === publishPVC) {
    expect(futurePVC).not.toBe(selectedPVC.code);
    console.log('✅ Rule 5 passed – no update');
  } else {
    expect(futurePVC).toBe(selectedPVC.code);
    expect(futureDate).toBe(selectedPVC.date);
    console.log('✅ PVC updated correctly');
  }
